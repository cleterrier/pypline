...
import numpy as np
from .loclib_psf2cspline import psf2cspline_np
...
class localizationlib:
    def __init__(self,usecuda=0):
        ...
        if usecuda==1:
            ...
            # Prefer the exact entry MATLAB uses; fall back if missing
            self._mleFit = getattr(lib_gpu_ast, 'GPUmleFit_LM_NoRestrict', lib_gpu_ast.GPUmleFit_LM)
    ...
    def loc_ast_dual(self, psf_data, cor, shared, imgcenter, T, fittype,
                     I_model=None, pixelsize_z=0.02, param_ratio=None, param_shift=None):
        ...
        if fittype == 2:
            # Check if I_model is already a precomputed spline coefficient array
            if isinstance(I_model, np.ndarray) and I_model.shape[1] == 64:
                # Already a spline coefficient stack
                Iall = I_model.astype(np.float32)
            else:
                # Generate spline coefficients from raw PSF stacks
                Iall = []
                Imd = I_model
                pbar = tqdm()
                for i in range(Nchannel):
                    ...
                    coeff = coeff[0] if isinstance(coeff, (list, tuple)) else coeff
                    Iall.append(coeff.astype(np.float32))
                    pbar.update(1)
                pbar.refresh()
                pbar.close()
            ...
            ccz = Iall.shape[-3] // 2
            initparam = np.array([-2, -1, 0, 1, 2]) * 0.15 / pixelsize_z + ccz
            ...
            splinesize = np.array([0]).astype(np.int32)
        ...
        if param_shift is not None:
            dTS[:, 0:Nchannel, :] += np.expand_dims(param_shift, axis=0)
        ...
        for param in paramstart:
            ...
            self._mleFit_MultiChannel(data, fittype, sharedA, iterations, Iall, dTS, varim, param,
                                       datasize, splinesize, Pk, CRLBk, LLk)
    ...
    # Individual gauss fit ready callable to GPU in batches
    def loc_2Dgauss_mleGPU(
        self,
        psf_data,                 # shape (N, 1, H, W)
        free_sigma=True,
        fixed_sigma=1.5,
        varmap=None,
        max_nfev=100,
        ...
        arr = np.asarray(psf_data, dtype=np.float32)
        if arr.ndim == 4:
            N, Z, H, W = arr.shape
            assert Z == 1, "Expected shape (N,1,H,W)"
            data = arr.reshape(N, H, W)
        else:
            raise ValueError("psf_data must be (N,1,H,W)")

        # Fit type
        fittype = 2 if free_sigma else 1
        coeff = np.array([fixed_sigma], dtype=np.float32)

        # Variance model (0 for EMCCD)
        varim = np.array(0, dtype=np.float32) if varmap is None else varmap.astype(np.float32)

        datasize = np.array([W, H, N], dtype=np.int32)
        splinesize = np.array([0], dtype=np.int32)

        Pk    = np.zeros((6, N), dtype=np.float32)
        CRLBk = np.zeros((5, N), dtype=np.float32)
        LLk   = np.zeros((N,), dtype=np.float32)

        iterations = np.int32(max_nfev)

        # Run DLL
        self._mleFit(data, fittype, iterations, coeff, varim, 0.0,
                    datasize, splinesize, Pk, CRLBk, LLk)

        center_x = (W - 1) / 2.0
        center_y = (H - 1) / 2.0

        dx = (Pk[1] - center_x).reshape(N, 1)
        dy = (Pk[0] - center_y).reshape(N, 1)

        loc_dict = dict(
            x=dx,
            y=dy,
            z=np.zeros((N, 1), dtype=np.float32),
            photons=Pk[2].reshape(N, 1),
            bg=Pk[3].reshape(N, 1),
            iterations=Pk[5].reshape(N, 1),
        )

        return Pk, CRLBk, LLk, loc_dict
